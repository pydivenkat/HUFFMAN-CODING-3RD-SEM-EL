...
int main()
{
	...
	else
	{
		...
		// Set file position of the stream to the beginningContains file signature
		offset = 0;
		...
		for (i = 0; i < height; i++)
		{
			for (j = 0; j < width; j++)
			{
				...
				// the Image is a 24-bit BMP Image
				temp = temp & 0x0000FF;
				image[i][j] = temp;
			}
		}
	...
	// Finding the probability of occurrence using histogram
	int hist[256];
	for (i = 0; i < 256; i++)
		hist[i] = 0;
	for (i = 0; i < height; i++)
	...
	pix_freq = (struct pixfreq*)malloc(sizeof(struct pixfreq) * totalnodes + 10);
	huffcodes = (struct huffcode*)malloc(sizeof(struct huffcode) * nodes + 10);
	...
	for (i = 0; i < 256; i++)
	{
		if (hist[i] != 0)
		{
			...
			pix_freq[j].freq = hist[i];
			huffcodes[j].freq = hist[i];
	...
	while (n < nodes - 1)
	{
		...
		// Adding the lowest two frequencies
		...
		// Sorting and Updating the huffcodes array simultaneously
	...
	FILE* imagehuff;
	char bin[]="";
	imagehuff = fopen("encoded.txt", "wb");
	for (i = 0; i < height; i++)
		for (j = 0; j < width; j++)
		{
			pix_val = image[i][j];
	...
	//Comparing with original image array
	for(int i=0;i<height;i++){
		for(int j=0;j<width;j++)
			printf("%d ",img[i][j]==image[i][j]);
		printf("\n");
	}
